...
def singleSymbol(geoTyp,props):
	...
	"""
	namCol = props
	symb = QgsSymbolV2.defaultSymbol(geoTyp)
	col = symb.color()
	col.setNamedColor(namCol)
	symb.setColor(col)
	# crea il renderer
	render = QgsSingleSymbolRendererV2(symb)
	return render
...
def catSymbol(geoTyp,attName,cats):
	...
	for i in cats:
		...
		col.setNamedColor(namCol)
		symb.setColor(col)
		cat = QgsRendererCategoryV2(name,symb,lab)
		catList.append(cat)
	# crea il renderer
	render = QgsCategorizedSymbolRendererV2(attName,catList)
	return render
...
def lsm(old,new):
	"""
		...
		questa routine non � ottimizzata ma � ripresa da uno studio di algebra lineare
		e si porta dietro tutto l'armamentario l� definito; potrebbe essere pesantemente
		riorganizzata
	"""
...
def erroreLsm(res,att):
	"""
		...
		il nome riferisce a LSM ma in realt� pu� essere usata per qualsiasi situazione
	"""
...
def openLibretto_contorni(file):
	"""
		...
		- ctrn[]	contorni particelle (lista [v0,v1,v2,....])
...
def lettureFraStazioni(libretto):
	"""
		...
		potrei usare la modularit� rispetto a 100, per� preferisco usare il codice di inizio riga
	"""
...
def elaborazioneGps(myGps):
	"""
		elaborazione dei gps (procedura perimentale)
...
def polar2rect(rilievo,a_giro):
	"""
		...
		NB: il senso di rotazione del teodolite Ã¨ inverso, quindi l'angolo va cambiato disegno
	"""
	coordRet = []
...
def allEsquad(archivio,allin):
	...
	if mu == 0.0:
		print "ERRORE: allineamento %s su %s(%f-%f)-%s(%f-%f) da un vettore nullo" % (cod,n1,x1,y1,n2,x2,y2)
...
#	printMatrix(mat)
	archivio = trasformaPunti3D(archivio,mat)	# non si pu� usare matrixMultiplication() perch� ci sono altri elementi da conservare
	return archivio
...
def collimazione3PF(oldCdsList,newCdsList,rilievo):
	"""
		...
		NB: occorre per forza LSM perch� sono 3 PF per 2 dim
	"""
	# mette a 0.0 la z
	for v in oldCdsList:
		v[2] = 0.0
...
#	printMatrix(mat)
	collimati = trasformaPunti2D(rilievo,mat)	# non si pu� usare matrixMultiplication() perch� ci sono altri elementi da 
	return collimati
...
class celerimensuraDlg(QDialog):
	"""
		...
		l'origine � nel punto a terra sotto la stazione
	"""
	...
	def redraw(self):
		...
		"""
		self.penEdge[0].setColor(QColor(0,0,0))	# la pen � l'elemento 0 della lista
		self.penEdge[0].setWidthF(1.5)
		self.penText[0].setColor(QColor(0,0,0))	# la pen � l'elemento 0 della lista
		# pulisce
		self.canvas.clear()
		# disegna l'origine
		...
		self.penEdge[0].setColor(QColor(255,0,0))	# la pen � l'elemento 0 della lista
...
#		----------- box principale ---------------
		...
		vBox.addLayout(hBox)
		btn = QPushButton("<")
		QObject.connect(btn,SIGNAL("clicked()"),self.prev)
		hBox.addWidget(btn)
		self.eLbl = QLabel(self)
		self.eLbl.setText('')
		hBox.addWidget(self.eLbl)
		btn = QPushButton(">")
		QObject.connect(btn,SIGNAL("clicked()"),self.next)
		hBox.addWidget(btn)
		...
		vBox.addLayout(hBox)
		btn = QPushButton("Go")
		QObject.connect(btn,SIGNAL("clicked()"),self.update)
		hBox.addWidget(btn)
...
#		con il clickTool qui non si riesce più a prendere il comando del mouse dopo aver eseguito un comando esterno
	...
	def initGui(self):
		...
		self.action = QAction(QIcon("Icons/topo4qgis.png"),"topog4qgis",self.iface.mainWindow())
...
#		-------- file menubar ------------
		...
		tmp.triggered.connect(self.importaLibretto)
		mFile.addAction(tmp)

		tmp = QAction(QIcon(''),'Importa EdM',self.dlg)        
		tmp.triggered.connect(self.importaEDM)
		mFile.addAction(tmp)
		...
		# ---------- referencng menu --------------
		mGeoref = mb.addMenu('Georeferencer')

		self.bGeoref = QAction(QIcon(''),'Georeferencer',self.dlg)        
		...
		QObject.connect(self.bAngle,SIGNAL("clicked()"),self.switchAngle)
...
#			print i
		...
		# attiva la label
		self.cLayer.label().setLabelField(0,0)
		self.cLayer.enableLabels(True)
		# rinfresca il video
		self.canvas.refresh()
...
#	--------------------- I/O functions -----------
	...
	def importaLibretto(self):
		...
		fname = QFileDialog.getOpenFileName(self.iface.mainWindow(),'Open file','/home/giuliano','*.dat')
		if fname:
			self.libretto = loadFile(fname)
...
#						print "punto ribattuto"
			...
			if len(self.RilCtrn):
				...
				self.cLayer.setRendererV2(myRen)
	...
	def importaEDM(self):
		...
		if len(self.edmPf):
			...
			if result == 2:
				...
				QgsMapLayerRegistry.instance().removeMapLayer(self.layEdmPf.id())
		# dialogo selezione file
		fname = QFileDialog.getOpenFileName(self.iface.mainWindow(),'Open file','/home/giuliano','*.emp')
		if fname:
			edm = loadFile(fname)
...
#			print "cerco i PF del foglio",fgCod,"del comune di",comCod
			# legge file
			fname = QFileDialog.getOpenFileName(self.iface.mainWindow(),'Open file','/home/giuliano','*.taf')
			if fname:
				# legge file
				f = open(fname, 'r')
				for data in f:
...
#						print 'matcha il comune',comCod
						if data[7:10] == fgCod:
#							print 'matcha il foglio',fg
							tmp = int(data[15:17])
							if tmp < 10:
								txt = "PF0%1s/%3s0/%5s" % (tmp,fgCod,comCod)
							else:
								txt = "PF%2s/%3s0/%5s" % (tmp,fgCod,comCod)
#							print txt
...
#			print "coordinate destinazione",newCds
			...
			QgsMapLayerRegistry.instance().removeMapLayer(self.layLibMisur.id())
			QgsMapLayerRegistry.instance().removeMapLayer(self.layLibRibat.id())
			QgsMapLayerRegistry.instance().removeMapLayer(self.layLibCtrn.id())
			# ---------------- ricrea i layer ----------------
...
#	---------- validation functions --------------------
	...
	def errorePF(self):
		"""
			...
			qu� � calcolato solo l'errore nel piano XY
		"""
		# controlla i PF misurati nel rilievo
		listaPf = pfLista(self.libretto)
		# --------- misurati --------
...
# --------- inquiry functions --------------------
	...
	def elencoPfUfficiali(self):
		print "Elenco dei punti fiduciali dellEdM"
